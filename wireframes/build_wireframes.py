...
PAGES = [
    dict(code="I.1", title="Реєстрація і верифікація телефону", short="Реєстрація і верифікація телефону", base="signup", group="I", job="ГОЛОВНИЙ, E1", flow="Flow 1 · вузол A", states=("успіх",), why="Flow 1 не дає інших станів після вузла A"),
    dict(code="I.2", title="Вибір ролі: шукаю кімнату / маю кімнату / обидва", short="Вибір ролі", base="role", group="I", job="ГОЛОВНИЙ", flow="Flow 1 · вузол B", states=("успіх",), why="прохідний вузол B без розвилок і станів"),
    dict(code="II.1", title="Створити / редагувати профіль", short="Створити / редагувати профіль", base="profile-edit", group="II", job="R2, S1, E2", flow="Онбординг · створення профілю", states=("успіх",), why="Flow 2–4 не дають окремих станів редагування профілю"),
    dict(code="II.2", title="Додати соцлінк для верифікації", short="Додати соцлінк для верифікації", base="profile-social", group="II", job="E1", flow="Flow 4 · верифікація до контакту", states=("успіх",), why="Flow 4 не дає окремих станів додавання соцлінку"),
    dict(code="III.1", title="Стрічка кімнат", short="Стрічка кімнат", base="listings", group="III", job="R1, R2, S1 (+R4)", flow="Flow 1 · вузол C", states=STATE_LABELS, why=""),
    dict(code="III.2", title="Стрічка людей-шукачів", short="Стрічка людей-шукачів", base="people", group="III", job="R2, ГОЛОВНИЙ (+R4)", flow="Flow 3 · вузли G–L", states=("успіх", "порожньо"), why="Flow 3 дає порожню видачу, але не дає помилки чи завантаження стрічки"),
    dict(code="III.3", title="Налаштувати фільтри", short="Налаштувати фільтри", base="filters", group="III", job="R1, R2, S1", flow="Flow 1 · вузол G / Flow 3 · вузол K", states=("успіх",), why="прохідний вузол фільтрів без власних станів"),
    dict(code="IV.1", title="Картка оголошення кімнати", short="Картка оголошення кімнати", base="listing", group="IV", job="ГОЛОВНИЙ, R2, E1", flow="Flow 1 · вузол I", states=("успіх", "помилка", "завантаження"), why="порожня картка не виникає: її відкривають з непорожньої стрічки"),
    dict(code="IV.2", title="Картка профілю людини", short="Картка профілю людини", base="person", group="IV", job="R2, E1, ГОЛОВНИЙ, S1", flow="Flow 3 · вузол M", states=STATE_LABELS, why=""),
    dict(code="V.1", title="Опублікувати оголошення", short="Опублікувати оголошення", base="new-listing", group="V", job="R5, ГОЛОВНИЙ", flow="Flow 3 · вузли C–F", states=("успіх", "помилка", "завантаження"), why="форма має контент, тому порожній стан не виникає"),
    dict(code="V.2", title="Мої оголошення", short="Мої оголошення", base="my-listings", group="V", job="R5, E1", flow="Flow 3 · вузли A–B", states=("успіх", "порожньо"), why="Flow 3 не дає помилки чи завантаження списку оголошень"),
    dict(code="VI.1", title="Заявки і чати (список)", short="Заявки і чати", base="chats", group="VI", job="R3", flow="Flow 2 · вузли A–I", states=STATE_LABELS, why=""),
    dict(code="VI.2", title="Чат — переписка після взаємної згоди", short="Чат — переписка після взаємної згоди", base="chat", group="VI", job="R3, E1 (+R4)", flow="Flow 1 · вузли S–T / Flow 2 · J–M", states=("успіх",), why="порожньо й тиша належать списку чатів у Flow 2"),
    dict(code="VI.3", title="Поскаржитись / заблокувати", short="Поскаржитись / заблокувати", base="report", group="VI", job="СИРОТА → E1", flow="Flow 4 · вузли L–M", states=("успіх",), why="Flow 4 веде з форми прямо до успішного розриву контакту"),
    dict(code="VII.1", title="Залишити відгук", short="Залишити відгук", base="review", group="VII", job="ГОЛОВНИЙ, E1", flow="Flow 4 · вузли P–Q", states=("успіх",), why="Flow 4 веде з форми прямо до успішного відгуку"),
]
...
def zone(name: str, role: str, action: str, body: str) -> str:
    return f'''<section data-zone="{name}">
  <p class="wf-note"><b>Роль:</b> {role} <b>Головна дія:</b> {action}</p>
  {body}
</section>'''
...
def state_switcher(page: dict, state: str) -> str:
    ...
    for label in STATE_LABELS:
        ...
        elif label in page["states"]:
            parts.append(f'<a href="{filename(page, label)}">{label}</a>')
        else:
            parts.append(f'<span class="is-absent" title="{escape(page["why"])}">{label} — {page["why"]}</span>')
    return "\n        ".join(parts)
...
def content_listing(state: str) -> str:
    ...
    if state == "успіх":
        ...
        body = '''<article class="wf-msg"><h3>Надсилання заявки…</h3><p>Заявку надіслано успішно?</p><button type="button" disabled>Надсилання заявки…</button><p class="wf-hint">Розвилки прототипу:</p><a href="listing-error.html">Ні, стався збій</a> · <a href="#waiting">Так, заявку надіслано</a></article><article class="wf-msg" id="waiting"><h3>Очікуємо відповідь на заявку…</h3><p>Контакти відкриються після взаємної згоди.</p><button type="button" disabled>Очікуємо відповідь…</button><p class="wf-hint">Розвилки прототипу:</p><a href="chat.html">Заявку прийнято</a> · <a href="listings.html">Заявку відхилено</a><p><a href="listings.html">Немає відповіді — заявка протухла</a></p></article>'''
    action = zone("головна дія", "показати рішення та стан заявки.", "надіслати заявку або вийти за гілкою Flow 1.", body)
    return appbar("Картка оголошення", "listings.html") + summary + trust + action + tabbar("search")


def content_chat(_: str) -> str:
    context = zone("контекст", "нагадати, навколо якого варіанта відкрито чат.", "повернутися до оголошення.", '''<article class="wf-card"><h3>Оболонь, 8&nbsp;500&nbsp;₴/міс</h3><p>Заявку прийнято — чат відкрито</p><a href="listing.html">Переглянути оголошення</a></article>''')
    conversation = zone("переписка", "підтримати безпечну розмову після взаємної згоди.", "обрати гілку домовленості або безпеки.", '''<article class="wf-bubble"><p>Привіт! Кімната ще вільна. Перед переглядом нічого переказувати не треба.</p></article><article class="wf-bubble wf-bubble-me"><p>Добре, у суботу о 15:00 мені підходить.</p></article><form><label class="wf-field" for="message"><span>Повідомлення</span><textarea id="message">Домовились, побачимось у суботу.</textarea></label><button type="button">Надіслати</button></form><article class="wf-msg"><h3>Просять передоплату до перегляду кімнати?</h3><p><a class="wf-btn wf-btn-ghost" href="report.html">Так — червоний прапорець: вимагають гроші наперед</a></p><p><a class="wf-btn wf-btn-primary wf-btn-block" href="review.html">Ні — домовились на особистий перегляд</a></p><p><a href="chats.html">Тиша або відмовки — розмова нічим не закінчилась</a></p></article><p><a href="my-listings.html">Мешканця знайдено — перейти до моїх оголошень</a></p>''')
    return appbar("Олег · Оболонь", "chats.html") + context + conversation + tabbar("chats")


def content_profile_edit(_: str) -> str:
    return appbar("Мій профіль") + zone("профіль", "дати іншим достатньо контексту для рішення.", "зберегти правдивий профіль.", '''<form><span class="ph ph-avatar" role="img" aria-label="місце для фото Марічки"></span><label class="wf-field" for="name"><span>Ім’я та вік</span><input id="name" value="Марічка, 21"></label><label class="wf-field" for="about"><span>Про себе</span><textarea id="about">Студентка КНУ, шукаю спокійний дім на Оболоні або Подолі.</textarea></label><label class="wf-field" for="profile-budget"><span>Бюджет</span><input id="profile-budget" value="до 9&nbsp;000&nbsp;₴/міс"></label><label class="wf-field" for="profile-areas"><span>Райони інтересу</span><input id="profile-areas" value="Оболонь, Поділ"></label><fieldset><legend>Звички</legend><label><input type="checkbox" checked> охайність: важлива</label><br><label><input type="checkbox" checked> гості: зрідка</label><br><label><input type="checkbox" checked> тварини: є кіт</label><br><label><input type="checkbox" checked> не палю</label><br><label><input type="checkbox"> жайворонок / сова</label></fieldset><button class="wf-btn-primary wf-btn-block" type="button">Зберегти профіль</button></form>''') + zone("верифікація", "показати рівень довіри до профілю.", "додати підтверджений соцлінк як другорядний крок.", '''<ul class="wf-chips"><li><span class="wf-badge">телефон підтверджено</span></li></ul><p><a class="wf-btn wf-btn-ghost wf-btn-block" href="profile-social.html">Додати соцлінк</a></p>''') + tabbar("profile")


def content_profile_social(_: str) -> str:
    return appbar("Верифікація соцлінком", "profile-edit.html") + zone("соцлінк", "додати ще один сигнал, що профіль належить реальній людині.", "підтвердити Instagram або LinkedIn.", '''<form action="profile-edit.html" method="get"><label class="wf-field" for="network"><span>Соцмережа</span><select id="network"><option>Instagram</option><option>LinkedIn</option></select></label><label class="wf-field" for="social"><span>Посилання</span><input id="social" type="url" value="https://instagram.com/marichka"></label><p class="wf-hint">Посилання видно як сигнал довіри; пароль Куток не просить.</p><button class="wf-btn-primary wf-btn-block" type="submit">Підтвердити соцлінк</button></form>''') + zone("наслідок", "пояснити результат до відправлення форми без передчасного badge.", "за потреби повернутися до профілю.", '''<p>Після підтвердження у профілі з’явиться сигнал «Instagram підтверджено».</p><p><a href="profile-edit.html">Скасувати й повернутися до профілю</a></p>''') + tabbar("profile")
...
def content_person(state: str) -> str:
    ...
    if state == "успіх":
        ...
        reviews = '<article class="wf-msg"><h3>Очікуємо відповідь на заявку…</h3><p>Профіль отримає пропозицію кімнати на Оболоні за 8&nbsp;500&nbsp;₴/міс.</p><button type="button" disabled>Очікуємо відповідь…</button><p class="wf-hint">Переходи прототипу:</p><a href="chat.html">Погодились оселитися</a> · <a href="person-error.html">Заявку відхилено</a><p><a href="people.html">Немає відповіді — повернутися до стрічки</a></p></article>'
    return appbar("Профіль людини", "people.html") + summary + zone("відгуки і дія", "показати репутацію та стан пропозиції.", "запропонувати кімнату або повернутися до пошуку.", reviews) + tabbar("people", True)
...
def content_new_listing(state: str) -> str:
    ...
    form = zone("оголошення", "зібрати факти про кімнату й повний набір побажань.", "перевірити дані перед публікацією.", form_body)
    if state == "успіх":
        ...
        body = '<article class="wf-msg"><h3>Публікуємо оголошення…</h3><p>Після збереження оголошення стане активним.</p><button type="button" disabled>Публікуємо оголошення…</button><p class="wf-hint">Перехід прототипу:</p><a href="my-listings.html">Показати опубліковане оголошення</a></article>'
    return appbar("Нове оголошення", "my-listings.html") + form + zone("публікація", "показати готовність або стан публікації.", "опублікувати чи виправити форму.", body) + tabbar("ads", True)
...
def content_my_listings(state: str) -> str:
    ...
    return appbar("Мої оголошення") + zone("перелік", "показати активні й закриті пропозиції або порожній стан.", "керувати оголошенням чи створити перше.", body) + tabbar("ads", True)
...
def content_chats(state: str) -> str:
    if state == "успіх":
        ...
        body = '<article class="wf-msg"><h3>Очікуємо відповідь на заявку…</h3><p>Відповіді від автора ще немає.</p><button type="button" disabled>Очікуємо відповідь…</button><p class="wf-hint">Розвилки прототипу:</p><a href="chat.html">Заявку прийнято</a> · <a href="chats-error.html">Заявку відхилено</a></article><article class="wf-msg wf-msg-dead"><h3>Заявка протухла без відповіді</h3><p>Ця лінія завершена без відповіді.</p><a href="listings.html">Шукати інший варіант</a></article>'
    return appbar("Заявки і чати") + zone("список розмов", "зібрати заявки та активні чати в одному місці.", "відкрити розмову або повернутися до пошуку.", body) + tabbar("chats")


def content_report(_: str) -> str:
    reason = zone("причина", "зібрати причину й факти для модерації.", "обрати причину скарги.", '''<fieldset><legend>Що сталося?</legend><label><input type="radio" name="reason" checked> Вимагають гроші наперед</label><br><label><input type="radio" name="reason"> Фейкове оголошення</label><br><label><input type="radio" name="reason"> Образи або спам</label></fieldset><label class="wf-field" for="details"><span>Деталі для модерації</span><textarea id="details">Автор просить передоплату до перегляду кімнати.</textarea></label>''')
    result = zone("результат", "пояснити наслідок захисної дії.", "надіслати скаргу й заблокувати профіль.", '''<p><b>Безпечно уникнула шахрайства, контакт розірвано.</b></p><p>Олег не зможе писати тобі, а модерація отримає скаргу.</p><button class="wf-btn-primary wf-btn-block" type="submit">Надіслати скаргу й заблокувати</button><p><a href="chat.html">Скасувати й повернутися в чат</a></p>''')
...
def content_review(_: str) -> str:
    experience = zone("досвід", "зафіксувати чесний досвід спільного проживання.", "описати досвід про людину.", '''<article class="wf-card"><h3>Олег, 29 · власник кімнати</h3><p>Оболонь, 8&nbsp;500&nbsp;₴/міс · заселення підтверджено</p></article><label class="wf-field" for="review"><span>Що варто знати майбутнім співмешканцям?</span><textarea id="review">Оголошення відповідало кімнаті, про умови домовились чесно.</textarea></label><label><input type="checkbox" checked> Умови оголошення були правдивими</label>''')
    publish = zone("публікація", "показати, як відгук підсилює довіру.", "опублікувати відгук і повернутися до профілю.", '''<p><b>Оселилася безпечно, лишила чесний відгук.</b></p><p>Відгук з’явиться у профілі Олега та вплине на бейдж «перевірено».</p><button class="wf-btn-primary wf-btn-block" type="submit">Опублікувати відгук</button>''')
...
{tree}
  <div class="wf-screen">
    <header class="wf-head">
      <h1>{page["title"]}</h1>
      <p class="wf-file">sitemap {page["code"]} · {current}</p>
      <dl class="wf-meta">
        <div><dt>Job:</dt><dd>{page["job"]}</dd></div>
        <div><dt>Flow:</dt><dd>{page["flow"]}</dd></div>
        <div><dt>Стан:</dt><dd>{state}</dd></div>
      </dl>
...
def render_index(available: set[str]) -> str:
    ...
    for group, label in GROUPS:
        ...
        for page in (item for item in PAGES if item["group"] == group):
            ...
            items.append(f'<li><strong>{page["code"]} · {page["title"]}</strong><ul>{states}</ul></li>')
        groups.append(f'<section data-zone="розділ {group.lower()}"><p class="wf-note"><b>Роль:</b> зібрати екрани розділу {group}. <b>Головна дія:</b> відкрити потрібний стан.</p><h2>{group}. {label}</h2><ul>{"".join(items)}</ul></section>')
    return f'''<!doctype html><html lang="uk"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1"><title>Куток · Всі вайрфрейми</title><link rel="stylesheet" href="wireframe.css"></head><body><div class="wf">{tree_html(None, available)}<div class="wf-screen"><header class="wf-head"><h1>Всі екрани і стани</h1><p class="wf-file">wireframes/index.html · 15 екранів</p></header><main class="wf-device">{"".join(groups)}</main></div></div></body></html>'''
